...
//make ner node for syntax setTypeStructOrEnumIdentifier
...
//make a new declarator for getIdentifierDeclared
...
A_TYPE *setTypeStructOrEnumfIdentifier(T_KIND k, char *s , ID_KIND kk) {
    A_TYPE *t;
    A_ID *id, *a;
...
A_TYPE *setTypeStructOrEnumIdentifier(T_KIND k, char *s, ID_KIND kk){
  A_TYPE *t;
  A_ID *id, *a;
  //check redeclaration or forward redeclaration
  a=searchIdentifierAtCurrentLevel(s, current_id);
  if(a){
    if(a->kind==kk && a->type->kind==k){
      if(a->type->field)
...
BOOLEAN isNotSameFormalParameters(A_ID *a, A_ID *b){
  if(a==NIL)//no paremeters in prototype
    return(FALSE);
  while(a){
